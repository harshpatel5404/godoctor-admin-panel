...
function initMap() {
    ...
    let zone = document.getElementById('old_lat_lon').value
    const jspzone = typeof zone == 'string' ? JSON.parse(zone) : zone
    let edit_zones = [];

    for (var i = 0; i < jspzone.length; i++){
        let object  = {lat: jspzone[i].latitude, lng: jspzone[i].longitiude}
